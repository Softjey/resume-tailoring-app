  ...
  try {
    ...
    if (useMock) {
      ...
      const webhookResponse = await fetch(
        "https://softjey.app.n8n.cloud/webhook/resume/tailor",
        {
          method: "POST",
      ...
      if (!webhookResponse.ok) {
        ...
        throw new Error(
          `Failed to process resume with webhook: ${webhookResponse.status}`
        );
      }

      const responseData = await webhookResponse.json();
      console.log(
        "[v0] Received response from webhook, has jsonResume:",
        !!responseData.jsonResume
      );
    ...
    console.log(
      `[v0] Successfully generated ${resumes.length} resume previews`
    );
