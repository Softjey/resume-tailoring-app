...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!theme || !id) {
      return NextResponse.json({ error: "Theme and ID parameters are required" }, { status: 400 })
    }
    ...
    if (!resume) {
      return NextResponse.json({ error: "Resume not found or expired. Please regenerate your resume." }, { status: 404 })
    }
